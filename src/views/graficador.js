...
class Graficador extends Component {
  ...
  async SetAlgoritoOperaciones() {
    ...
    datos.forEach(wits => {
      ...
      for (const opt in selects) {
        ...
        switch (
          ...
          case 'wd':
            ...
            break;
          case 'ls':
            const [datolas] = this.state.dataArchivosCurvas
              .filter(c => c.id == origen[1])
              .map(d => ({ datos: d.datos }));
            const [datosfilter] = datolas.datos.filter(
              e => e['DATETIME'] === wits.DATETIME
            );
            if (datosfilter != undefined) {
              const dato = datosfilter['_' + origen[2]];
              algoritmo[index] = dato;
            } else algoritmo[index] = 0;
      ...
      let operacion = AlgoritmoOperaciones(
        DBTM_0,
        algoritmo[1],
        algoritmo[0],
        algoritmo[5],
      ...
      if (operacion === 0) {
        operacion_0.push(DBTM);
      } else {
        operacion_0.push(null);
      }
      if (operacion === 2) {
        operacion_2.push(DBTM);
      } else {
        operacion_2.push(null);
      }
      if (operacion === 3) {
        operacion_3.push(DBTM);
      } else {
        operacion_3.push(null);
      }
      if (operacion === 4) {
        operacion_4.push(DBTM);
      } else {
        operacion_4.push(null);
      }
      if (operacion === 7) {
        operacion_7.push(DBTM);
      } else {
        operacion_7.push(null);
      }
      if (operacion === 8) {
        operacion_8.push(DBTM);
      } else {
        operacion_8.push(null);
      }
      if (operacion === 9) {
        operacion_9.push(DBTM);
      } else {
        operacion_9.push(null);
      }
      if (operacion === 35) {
        operacion_35.push(DBTM);
      } else {
        operacion_35.push(null);
      }
      if (operacion === 36) {
        operacion_36.push(DBTM);
      } else {
        operacion_36.push(null);
      }
      if (operacion === 37) {
        operacion_37.push(DBTM);
      } else {
        operacion_37.push(null);
      }
      if (operacion === 38) {
        operacion_38.push(DBTM);
      } else {
        operacion_38.push(null);
      }
      if (operacion === 39) {
        operacion_39.push(DBTM);
      } else {
        operacion_39.push(null);
      }
      if (operacion === 40) {
        operacion_40.push(DBTM);
      } else {
        operacion_40.push(null);
      }
      if (operacion === 41) {
        operacion_41.push(DBTM);
      } else {
  ...
  async EjecutarAlgoritmo() {
    ...
    for (const opt in selects) {
      if (selects[opt] === '0') validar = false;
    }
  ...
  LimparAlgoritmo = () => {
    ...
    const opx = this.state.dataConvencion.map(con => ({ name: con.nombre }));
    const g_opx = this.state.dataGP.filter(c => {
      return opx.find(x => x.name === c.name);
    });
    if (g_opx.length > 0) {
      let data = [...this.state.dataGP];
      let layout = { ...this.state.layoutGP };
      g_opx.forEach(e => {
        ...
        if (index_d >= 0) data.splice(index_d, 1);
      });
      this.setState({
        dataGP: data,
        layoutGP: layout,
  ...
  toggleEventos = () => {
    ...
    const tvs = this.state.dataTipoEvento.map(te => ({ name: te.label }));
    const evts = this.state.dataGP.filter(c => {
      return tvs.find(x => x.name === c.name);
    });
  ...
  toggleModalAlgoritmo = show => {
    if (show) {
      ...
      this.setState({
        optionsDetalle: op_wd,
        alert_algoritmo: { show: false },
      });
    }
  ...
  handleConfigChangePrincipal = e => {
    ...
    p.forEach(r => {
      if (r.id == ids[1]) r.mostrar = !r.mostrar;
    });
  ...
  handleConfigChangeHorizontal = (e, esCheck) => {
    ...
    h.forEach(r => {
      if (r.id == ids[1]) {
        if (esCheck) r.mostrar = !r.mostrar;
        else r.grupo = e.target.value ? e.target.value : 1;
      }
    });
    ...
    a.forEach(arc => {
      if (arc.id == ids[1]) {
        arc.homologacion.forEach(h => {
          if (h.id == ids[2]) h.mostrar = !h.mostrar;
        });
      }
    });
    this.setState({
  ...
  getTemplates = id => {
    axios
      .get(URL + 'templates_wells/well/' + id)
      .then(response => {
        this.setState({
          dataTemplates: response.data,
          loadingTemplates: false,
        });
      })
      .catch(error => {
  ...
  getConvencion = async () => {
    axios
      .get(URL + 'convencion_datos_operacion')
      .then(response => {
  ...
  getArchivos = async id => {
    ...
    axios
      .get(URL + 'archivo_encabezado/well/' + id)
      .then(response => {
        ...
        archivos_las.forEach(row => {
          ...
          this.getDataArchivos(row.id, Number(row.es_tiempo)).then(resp => {
            this.setState(prev => ({
              dataArchivosCurvas: [...prev.dataArchivosCurvas, resp],
            }));
          });
        });
  ...
  getDataArchivos = (id, tipo) => {
    return new Promise((resolve, reject) => {
      axios
        .get(URL + `archivo_encabezado/datawell/${id}/${tipo}`)
        .then(response => {
          const datos = {
            id: id,
            datos: response.data,
    ...
    switch (tg) {
      case 1:
        this.setState({
          ...
          toggleTrackHorizontal: false,
          plotDeptCol: this.state.toggleTrackVertical
            ? 'col-md-8'
            : 'col-md-12',
          plotDeptVH: '95vh',
          plotTrackVH: '0vh',
            ...
            if (this.state.dataWits.length === 0) {
              axios
                ...
                .then(response => {
                  ...
                  const datosToSimplyfy = data_Wits.map(e => ({
                    x: e['id'],
                    y: Number(e['_0108']),
                  }));
                  const dataSimplyfy = Simplify(datosToSimplyfy, 0.9075).map(
                    prop => prop.x
                  );

                  const dataFilter_Wits = data_Wits.filter(item =>
                    dataSimplyfy.includes(item.id)
                  );
                  ...
                  //Gráfica principal
                  let curvasPrincipal = curvas.filter(
                    c => c.mostrar === true && c.grupo === null
                  );
                  this.SetSerieGraficaPrincipal(curvasPrincipal);
                  //

                  //Track Horizontal
                  let traksHorizontales = curvas
  ...
  SetSerieGraficaPrincipal = curvas_p => {
    let data = [...this.state.dataGP];
    let layout = { ...this.state.layoutGP };
    let profundidadFinal_temporal = layout.yaxis.range
    ...
    curvas_p.map(c => {
      if (c.mostrar) {
        ...
        if (index === -1) {
          //Agregar curva
          const datos = this.state.dataWits.map(f => ({
            x: f['DATETIME'],
            y: f['_' + c.codigo],
          }));
          ...
          const serie = {
            ...
            type: 'scatter',
            text:
              '[' + c.codigo + '] ' + c.short_mnemonico + ' - ' + c.descripcion,
          };
          data.push(serie);

          let prof = Math.max(...y) + 500;
          profundidadFinal_temporal =
            prof > profundidadFinal_temporal ? prof : profundidadFinal_temporal;
    ...
    curvas_h.map(c => {
      if (c.mostrar) {
        ...
        if (c.grupo == grupo_anterior)
          layout_Horizontal[propertyAxi] = {
            title: c.short_mnemonico,
            titlefont: { size: 10, color: '#3c5cf9' },
            tickfont: { size: 8.0 },
            overlaying: 'y' + (i > 1 ? String(i - 1) : ''),
            side: 'right',
            gridcolor: '#eee',
          };
        else {
    ...
    this.state.archivos_las.forEach(ar => {
      if (ar.es_tiempo === true) {
        ar.homologacion.forEach(hm => {
          if (hm.mostrar) {
            const [curvas] = this.state.dataArchivosCurvas
              .filter(c => c.id === ar.id)
              .map(d => ({ datos: d.datos }));
            const datos = curvas.datos.map(f => ({
              x: f['DATETIME'],
              y: f['_' + hm.codigo],
            }));
            const x = datos.map(d => d.x);
  ...
  SetSerieTrackVertical = () => {
    ...
    else newMax = this.MaxTrackVertical(-20, this.state.profundidadFinal);
        ...
        ar.homologacion.forEach(hm => {
          if (hm.mostrar) {
            const [curvas] = this.state.dataArchivosCurvas
              .filter(c => c.id === ar.id)
              .map(d => ({ datos: d.datos }));
            const datos = curvas.datos
              .filter(x => x['_' + hm.codigo] !== '-999.25')
              .map(f => ({ y: f['DEPTH'], x: f['_' + hm.codigo] }));
            const x = datos.map(d => d.x);
            const y = datos.map(d => d.y);
  ...
  handleChangeTipoEvento = e => {
    let tipoEvento = this.state.dataTipoEvento.find(
  ...
  handlePlot = () => {
    ...
    this.setState({
      evento: {
        ...
        hora_inicial: fecha[1],
        profundidad_inicial: String(punto.prof),
        profundidad_final: '',
      },
    });
    this.setState({
      menuEmergente: { showMenu: false },
      insertar: true,
      openModalEvento: true,
    });
  };
  ...
  ConfirmDeleteTemplate = () => {
    ...
    axios
      .delete(URL + 'templates', { data: datos })
      .then(response => {
  ...
  ActualizaTemplate = () => {
    if (this.state.dataCurvas.length > 0) {
      ...
      axios
        .put(URL + 'templates_wells_wits_detalle_secciones', data)
        .then(response => {
  ...
  UpdateEvent = () => {
    ...
    let fechaFinal =
      this.state.evento.fecha_final + ' ' + this.state.evento.hora_final;
    ...
    if (this.state.showing) this.state.evento.tipo_tiempo = 2;
    else {
      ...
      this.state.evento.profundidad_final =
    ...
    //Actualizar
    axios
      .put(URL + 'eventos', this.state.evento)
      .then(response => {
        let updateGrafica = false;
        let newData = [...this.state.dataGP];
        let index = this.state.dataGP.findIndex(
          item => item.name === anterior.TipoEvento
        );
        ...
        if (anterior.tipo_evento_id === evento.tipo_evento_id) {
          ...
          if (anterior.tipo_tiempo === 1) {
            ...
            if (evento.tipo_tiempo === 2) {
              //Actualizar el shape
              let indexLayout = newLayout.shapes.findIndex(
                shape => shape.name === evento.id
              );
          ...
          let nextIndex = newData[index].text.findIndex(
            subitem => subitem === evento.id
          );

          //si es único registro, remover la traza
          if (newData[index].x.length === 0) newData.splice(0, 1);
          else {
          ...
          let indexNew = newData.findIndex(
            item => item.name === evento.TipoEvento
          );
          ...
          if (indexNew >= 0) {
            ...
            let traceEvento = {
              ...
              mode: 'markers',
              type: 'scatter',
              marker: { symbol: '303', size: 10, color: color },
              hovertemplate: '%{x}, %{y}',
              text: t,
            };
        ...
        } else this.setState({ openModalEvento: false });
  ...
  InsertEvent = () => {
    ...
    axios
      .post(URL + 'eventos', this.state.evento)
      .then(response => {
      ...
      <div className="App">
          ...
          <div className="row border-bottom bg-verdeclaro">
            ...
            <div className="col-md-4 col-lg-4 text-left  mt-1">
              ...
              <button
                ...
                onClick={() => this.ActualizaTemplate()}
                className="btn btn-sm btn-success btn-circle"
              >
                <Save fontSize="small" />
              </button>
              ...
              <button
                title="Dark Mode"
                onClick={() => this.DarkSide()}
                className="btn btn-sm btn-dark btn-circle"
              >
                <InvertColorsOff fontSize="small" />
              </button>
              &nbsp;&nbsp;|&nbsp;&nbsp;
              <button
                title="Algoritmo de Operaciones"
                onClick={() => this.toggleModalAlgoritmo(true)}
              ...
              &nbsp;&nbsp;|&nbsp;&nbsp;
              <button
                title="Ver/Ocultar Eventos"
                onClick={() => this.toggleEventos()}
              ...
              &nbsp;&nbsp;
              <button
                title="Ver/Ocultar Operaciones"
                onClick={() => this.toggleOperaciones()}
              ...
              >
                <HorizontalSplit fontSize="small" className="rotate" />
              </button>
              &nbsp;&nbsp;
              <button
        ...
        <Menu
          ...
          anchorPosition={
            this.state.menuEmergente.y !== 0 && this.state.menuEmergente.x !== 0
              ? {
                  top: this.state.menuEmergente.y + 10,
                  left: this.state.menuEmergente.x + 10,
                }
              : undefined
          }
        >
          <MenuItem key={'MI1'} onClick={this.handlePlot}>
            <EventAvailable fontSize="small" /> Agregar Evento
          </MenuItem>
          <MenuItem key={'MI2'} onClick={this.handleClose}>
          ...
          <ModalBody>
              ...
              <div className="col-md-10">
                <div className="form-group">
                  ...
                  <select
                    ...
                    defaultValue={
                      this.state.evento ? this.state.evento.tipo_evento_id : ''
                    }
                  >
                    {this.state.insertar ? (
            ...
            <div className="row">
              <div className="col-md-4">
                <div className="form-group">
                  <label>
                    <b>
                      <EventAvailable fontSize="small" /> Fecha Inicial:{' '}
                    </b>
                  </label>
                  <TextField
                    id="fecha_inicial"
                    ...
                    type="date"
                    defaultValue={
                      this.state.evento ? this.state.evento.fecha_inicial : ''
                    }
                    className="form-control"
                    onChange={this.handleChangeEvento}
                    InputLabelProps={{
              ...
              <div className="col-md-4">
                <div className="form-group">
                  ...
                  <TextField
                    ...
                    type="time"
                    defaultValue={
                      this.state.evento ? this.state.evento.hora_inicial : ''
                    }
                    className="form-control"
                    onChange={this.handleChangeEvento}
                    InputLabelProps={{
            ...
            <div className="row" onClick={() => this.setFechaFinal()}>
              <div className="col-md-12 text-center">
                <div className="form-group">
                  <CalendarToday style={{ cursor: 'pointer' }} /> Click para{' '}
                  {this.state.showing ? 'inactivar' : 'activar'} fecha final
                </div>
              </div>
            </div>
                ...
                <div className="col-md-4">
                  <div className="form-group">
                    ...
                    <TextField
                      ...
                      type="date"
                      defaultValue={
                        this.state.evento ? this.state.evento.fecha_final : ''
                      }
                      className="form-control"
                      onChange={this.handleChangeEvento}
                      InputLabelProps={{
                ...
                <div className="col-md-4">
                  <div className="form-group">
                    ...
                    <TextField
                      ...
                      type="time"
                      defaultValue={
                        this.state.evento ? this.state.evento.hora_final : ''
                      }
                      className="form-control"
                      onChange={this.handleChangeEvento}
                      InputLabelProps={{
              ...
              <div className="col-md-12">
                <div className="form-group">
                  ...
                  <textarea
                    ...
                    name="descripcion"
                    value={
                      this.state.evento ? this.state.evento.descripcion : ''
                    }
                    className="form-control"
                    onChange={this.handleChangeEvento}
                  />
          ...
          <ModalFooter>
            ...
            {this.state.insertar ? (
              ...
              <button
                ...
                onClick={() => this.setState({ openModalDeleteEvento: true })}
              >
                <Delete /> Eliminar{' '}
              </button>
            )}
            <button
              ...
              onClick={() => this.setState({ openModalEvento: false })}
            >
              <Close /> Cerrar
            </button>
          </ModalFooter>
        ...
        <Modal isOpen={this.state.openModalDeleteEvento}>
          <ModalBody>
            ...
            <div className="form-group">
              ...
              {this.state.evento.fecha_inicial +
                ' ' +
                this.state.evento.hora_inicial}
              {this.state.evento.tipo_tiempo == 2
                ? ' ~ ' +
                  this.state.evento.fecha_final +
                  ' ' +
                  this.state.evento.hora_final
                : ''}
              <br />
              <b>
          ...
          <ModalFooter>
            ...
            <button
              className="btn btn-secondary"
              onClick={() => this.setState({ openModalDeleteEvento: false })}
            >
              {' '}
              No
            </button>
        ...
        <Modal isOpen={this.state.openModalOperacion} size="lg">
              ...
              <div className="col-md-6">
                <div className="form-group">
                  ...
                  <TextField
                    ...
                    disabled
                    defaultValue={
                      this.state.operacion ? this.state.operacion.desde : ''
                    }
                    className="form-control"
              ...
              <div className="col-md-6">
                <div className="form-group">
                  ...
                  <TextField
                    ...
                    disabled
                    defaultValue={
                      this.state.operacion ? this.state.operacion.md_from : ''
                    }
                    className="form-control"
                    onChange={this.handleChangeOperacion}
                    InputLabelProps={{
              ...
              <div className="col-md-6">
                <div className="form-group">
                  ...
                  <TextField
                    ...
                    disabled
                    defaultValue={
                      this.state.operacion ? this.state.operacion.hasta : ''
                    }
                    className="form-control"
                    onChange={this.handleChangeOperacion}
                    InputLabelProps={{
              ...
              <div className="col-md-4">
                <div className="form-group">
                  ...
                  <TextField
                    ...
                    disabled
                    defaultValue={
                      this.state.operacion ? this.state.operacion.md_to : ''
                    }
                    className="form-control"
                    onChange={this.handleChangeOperacion}
                    InputLabelProps={{
              ...
              <div className="col-md-12">
                <div className="form-group">
                  ...
                  <textarea
                    ...
                    disabled
                    value={
                      this.state.operacion ? this.state.operacion.operacion : ''
                    }
                    className="form-control"
                    onChange={this.handleChangeOperacion}
                  />
          ...
          <ModalFooter>
            <button
              className="btn btn-secondary"
              onClick={() => this.setState({ openModalOperacion: false })}
            >
              <Close /> Cerrar
            </button>
          </ModalFooter>
        ...
        <Modal isOpen={this.state.modalStart}>
                                ...
                                <tr key={'row_' + index}>
                                  ...
                                  <td>
                                    <button
                                      ...
                                      title="Eliminar"
                                      onClick={() =>
                                        this.EliminarTemplate(row.template_id)
                                      }
                                    >
          ...
          <ModalFooter>
            ...
            <button className="btn btn-primary" onClick={() => this.IrHome()}>
              <Home /> Salir
            </button>
            <button
              className="btn btn-secondary"
              onClick={() =>
                this.setState({ modalStart: false, loadingStart: false })
              }
            >
              <Close /> Cerrar
            </button>
              ...
              <div className="col-md-12">
                <div className="form-group">
                  ...
                  <input
                    ...
                    id="tmpNombre"
                    className="form-control"
                    onChange={this.handleChangeNuevo}
                  />
                </div>
              </div>
            </div>
        ...
        <Modal isOpen={this.state.openModalDeleteTemplate}>
          ...
          <ModalFooter>
            ...
            <button
              className="btn btn-secondary"
              onClick={() => this.setState({ openModalDeleteTemplate: false })}
            >
              {' '}
              No
            </button>
        ...
        <Modal isOpen={this.state.modalConfig} size="lg">
          <ModalHeader>
            <div className="row">
              <div className="col-md-1">
                <Brush
                  fontSize="large"
                  className="btn-circle bg-primary text-white"
                />
              </div>
            ...
            <Accordion defaultActiveKey="0">
              <Card>
                <Accordion.Toggle
                  as={Card.Header}
                  eventKey="0"
                  className="bg-verdeoscuro small cursor-pointer"
                >
                  GRÁFICA PRINCIPAL
                </Accordion.Toggle>
                      ...
                      ? this.state.principal.map(tipo => (
                          <div key={'dgp_' + tipo.id} className="form-check">
                            <input
                              ...
                              type="checkbox"
                              checked={tipo.mostrar}
                              onChange={e =>
                                this.handleConfigChangePrincipal(e)
                              }
                              id={`chkgp_${tipo.id}`}
                            />
                            <label
                              className="form-check-label"
                              htmlFor={`chkgp_${tipo.id}`}
                            >
                              {'[' +
                                tipo.codigo +
                                '] ' +
                                tipo.short_mnemonico +
                                ' - ' +
                                tipo.descripcion}
                            </label>
                          </div>
              ...
              <Card>
                <Accordion.Toggle
                  as={Card.Header}
                  eventKey="1"
                  className="bg-verdeclaro  small cursor-pointer"
                >
                  TRACKS HORIZONTALES
                </Accordion.Toggle>
                  ...
                  <Card.Body>
                    <table className="table table-sm">
                      <tbody>
                        <tr>
                          <td></td>
                          <td style={{ widt: '50px' }}># de Track</td>
                        </tr>
                          ...
                          <tr key={'gth_' + tipo.id}>
                            <td>
                              <div className="form-check">
                                <input
                                  ...
                                  checked={tipo.mostrar}
                                  onChange={e =>
                                    this.handleConfigChangeHorizontal(e, true)
                                  }
                                  id={`chkth_${tipo.id}`}
                                />
                                <label
                                  className="form-check-label"
                                  htmlFor={`chkth_${tipo.id}`}
                                >
                                  {'[' +
                                    tipo.codigo +
                                    '] ' +
                            ...
                            <td>
                              <input
                                ...
                                id={`grup_${tipo.id}`}
                                onChange={e =>
                                  this.handleConfigChangeHorizontal(e, false)
                                }
                              />
                            </td>
                          </tr>
              ...
              <Card>
                ...
                <Accordion.Collapse eventKey="2">
                  <Card.Body>Caving 1 , 2 , 3,</Card.Body>
                </Accordion.Collapse>
              </Card>
                        ...
                        {this.state.archivos_las.map(arc => (
                          <tr key={'tr_' + arc.id}>
                            <td>
                              ...
                              <br />
                              <b>NOMBRE:</b>
                              <br /> {arc.nombre_archivo} <br />
                              <br />
                              <b>REFERENCIA:</b>
                              <br />
                              {arc.es_tiempo ? 'TIEMPO' : 'PROFUNDIDAD'}
                            </td>
                            <td>
                              <b>HOMOLOGADOS:</b>
                              {arc.homologacion.map(key => (
                                ...
                                >
                                  <input
                                    className="form-check-input"
                                    type="checkbox"
                                    id={`chklas_${arc.id}_${key.id}`}
                                    name={`chklas_${arc.id}_${key.id}`}
                                    checked={key.mostrar}
                                    onChange={e => this.handleChangeLas(e)}
                                  />
                                  <label
                                    className="form-check-label"
                                    htmlFor={`chk_lt_${key.id}`}
                                  >
                                    {key.nombre} ={' '}
                                    {'[' +
                                      key.codigo +
          ...
          <ModalFooter>
            ...
            <button
              className="btn btn-success"
              onClick={() => this.toggleModalTemplate(false, true)}
            >
              <PlaylistAddCheck /> Aplicar
            </button>
            <button
              className="btn btn-secondary"
              onClick={() => this.toggleModalTemplate(false, false)}
            >
              <Close /> Cerrar
            </button>
          </ModalFooter>
        ...
        <Modal isOpen={this.state.modalAlgoritmo} size="lg">
          <ModalHeader>
            <MultilineChart
              fontSize="large"
              className="btn-circle bg-warning"
            />{' '}
            <b>Algoritmo de Operaciones</b>
          </ModalHeader>
          <ModalBody>
            <div className="row">
              <div className="col-md-12">
                    ...
                    <tr>
                      ...
                      <td>
                        <select
                          ...
                          id="wd_0108"
                          name="wd_0108"
                          onChange={this.handleChangeAlgoritmo}
                          defaultValue={this.state.algoritmo.wd_0108}
                        >
                    ...
                    <tr>
                      ...
                      <td>
                        <select
                          ...
                          name="wd_0110"
                          onChange={this.handleChangeAlgoritmo}
                          defaultValue={this.state.algoritmo.wd_0110}
                        >
                          <option value="0">Seleccione</option>
                          {this.state.optionsDetalle
                            ? this.state.optionsDetalle.map(opt => (
                    ...
                    <tr>
                      ...
                      <td>
                        <select
                          ...
                          name="wd_0113"
                          onChange={this.handleChangeAlgoritmo}
                          defaultValue={this.state.algoritmo.wd_0113}
                        >
                          <option value="0">Seleccione</option>
                          {this.state.optionsDetalle
                            ? this.state.optionsDetalle.map(opt => (
                    ...
                    <tr>
                      ...
                      <td>
                        <select
                          ...
                          name="wd_0116"
                          onChange={this.handleChangeAlgoritmo}
                          defaultValue={this.state.algoritmo.wd_0116}
                        >
                          <option value="0">Seleccione</option>
                          {this.state.optionsDetalle
                            ? this.state.optionsDetalle.map(opt => (
                    ...
                    <tr>
                      ...
                      <td>
                        <select
                          ...
                          name="wd_0118"
                          onChange={this.handleChangeAlgoritmo}
                          defaultValue={this.state.algoritmo.wd_0118}
                        >
                          <option value="0">Seleccione</option>
                          {this.state.optionsDetalle
                            ? this.state.optionsDetalle.map(opt => (
                    ...
                    <tr>
                      ...
                      <td>
                        <select
                          ...
                          name="wd_0120"
                          onChange={this.handleChangeAlgoritmo}
                          defaultValue={this.state.algoritmo.wd_0120}
                        >
                          <option value="0">Seleccione</option>
                          {this.state.optionsDetalle
                            ? this.state.optionsDetalle.map(opt => (
                    ...
                    <tr>
                      ...
                      <td>
                        <select
                          ...
                          name="wd_0130"
                          onChange={this.handleChangeAlgoritmo}
                          defaultValue={this.state.algoritmo.wd_0130}
                        >
                          <option value="0">Seleccione</option>
                          {this.state.optionsDetalle
                            ? this.state.optionsDetalle.map(opt => (
          ...
          <ModalFooter>
            ...
            <button
              className="btn btn-success"
              onClick={async () => this.EjecutarAlgoritmo()}
            >
              <SlowMotionVideo /> Ejecutar
            </button>
            <button
              className="btn btn-warning"
              onClick={() => this.LimparAlgoritmo()}
            >
              <LayersClear /> Limpiar Operaciones
            </button>
            <button
              className="btn btn-secondary"
              onClick={() => this.toggleModalAlgoritmo(false)}
            >
              <Close /> Cerrar
            </button>
          </ModalFooter>
