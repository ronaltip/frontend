...
class Graficador extends Component {
    ...
    getTemplates = (id) => {
        axios.get(URL + 'templates_wells/' + id + '/' + 2 ).then(response => {
    ...
    AbrirTemplate = (id) => {
        ...
        axios.all([requestTemplatesWells, requestTemplatesWellTipoCurva]).then(axios.spread((...response) => {
            ...
            axios.get(URL + "datos_wits/wells/" + template.wells_id).then(response => {
                const dataWits = response.data.filter(d=> Number(d['0108']) < 15000 && Number(d['0110']) < 15000  && Number(d['0108']) > 0 && Number(d['0110']) > 0) ;
                this.setState({dataWits: dataWits})
                if (curvas.length > 0)
                {
                    //Gráfica principal
                    curvas.filter(c=>c.mostrar ===true && c.grupo===null).map(c => {
                        
                        const datos = dataWits.map( f => ({ x: f['DATETIME'], y: f[c.codigo]}) );  
            ...
            <div className="App">
                ...
                <div className="container-fluid">
                    ...
                    <div className="row border-bottom">
                        ...
                        <div className="col-md-4 col-lg-4 text-left  mt-1">
                            <button title="Abrir Template" onClick={() => this.Start()} className="btn btn-sm btn-outline-success btn-circle"><FolderOpenIcon fontSize="small" /></button>
                            &nbsp;&nbsp;
                            <button title="Guardar Template" onClick={() => this.GuardarTemplate()} className="btn btn-sm btn-success btn-circle"><SaveIcon fontSize="small" /></button>
                            &nbsp;&nbsp;|&nbsp;&nbsp;
                            <button title="Configuración" onClick={() => this.toggleModalTemplate(true, false, null)} className="btn btn-sm btn-primary btn-circle"><BrushIcon fontSize="small" /></button>
                            &nbsp;&nbsp;|&nbsp;&nbsp;
                            <button title="Ver/Ocultar Tracks Horizontales" onClick={() => this.CollapseTrackHorizontal()} className="btn btn-sm btn-outline-info btn-circle"><HorizontalSplitIcon fontSize="small" /></button>
                            &nbsp;&nbsp;
                            <button title="Ver/Ocultar Tracks Verticales" onClick={() => this.CollapseTrackVertical()} className="btn btn-sm btn-outline-danger btn-circle"><BarChartIcon fontSize="small" /></button>
                        </div>
                ...
                <Modal isOpen={this.state.modalStart} >
                        ...
                        <div className="row">
                            <div className="col-md-5">
                                <div className="form-group">
                                    ...
                                    <select name="field_id" id="wells_id" className="form-control" onChange={this.handleChange} defaultValue={template ? template.field_id : 0}>
                                            ...
                                            <tbody>
                                                {
                                                    ...
                                                    this.state.dataTemplates.map( (row , index) => 
                                                        <tr key={'row_'+index}>
                                                            ...
                                                            <td >
                                                                <button id={'btnopen_'+index} className="btn btn-sm btn-block btn-success" onClick={()=> this.AbrirTemplate(row.id)}>Abrir</button>
                                                            </td>
                                                            <td >
                                                                <button id={'btndel_'+index} className="btn btn-sm btn-block btn-danger" onClick={()=> this.EliminarTemplate(row.id)}>Eliminar</button>
                                                            </td>
                                                        </tr>
                                                    )
                                                    :
                    ...
                    <ModalFooter>
                        ...
                        <button className="btn btn-light" onClick={() => this.setState({modalStart : false}) }>Cancelar</button>
                        <button className="btn btn-secondary" onClick={() => this.IrHome()}>Cerrar</button>
                    </ModalFooter>
                </Modal>
                ...
                <Modal isOpen={this.state.modalNuevo} >
                    ...
                    <ModalFooter>
                        <button className="btn btn-success"   onClick={() => this.GuardarTemplate()}>Guardar</button>
                        <button className="btn btn-secondary" onClick={() => this.Start()}>Cancelar</button>
                    </ModalFooter>
                </Modal>
