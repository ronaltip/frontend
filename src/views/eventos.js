...
const tableIcons = {
    ...
    CalendarTodayIcon: forwardRef((props, ref) => <CalendarTodayIcon {...props} ref={ref} />)
}
...
class viewEventos extends Component {
    ...
    state = {
        ...
        modalEliminar: false,
        form: { id: '', wells_id: '', tipo_evento_id: '', tipo_tiempo: 1, fecha_inicial: '', hora_inicial: '', fecha_final: '', hora_final: '', profundidad_inicial: '', profundidad_final: '', descripcion: '', causa: '', solucion: '', estado_id: '', pkuser: '', nombre_tipo_eventos: '', nombre_pozo: '' },
    ...
    peticionGet = async () => {
        axios.get(URL + 'eventos').then(response => {
    ...
    peticionPost = async () => {
        ...
        await axios.post(URL + 'eventos', this.state.form ).then(response => {
            this.modalInsertar();
            this.peticionGet();
        }).catch(error => {
            console.log(error.message);
    ...
    modalInsertar = (op) => {
        ...
        {
            case 1:
                this.setState({ form: null, tipoModal: 'insertar', openModalInsertar: true, showing: false }); 
                break;
            case 2:
        ...
        return (
            <div className="App">
                ...
                <div className="containerCuatro">
                    <button className="btn btn-success" onClick={() => { this.modalInsertar(1) }}> <AddBox icon={tableIcons.Add} /> Agregar Eventos</button>
                </div>
                    ...
                    actions={[
                        {
                            ...
                            tooltip: 'Editar',
                            onClick: (event, rowData) => { this.seleccionarRegistro(rowData); this.modalInsertar(2) }
                        },
                        {
                ...
                <Modal isOpen={this.state.openModalInsertar}>
                    ...
                    <ModalBody>
                        <div className="form-group">
                            ...
                            <select name="tipo_evento_id" id="tipo_evento_id" className="form-control" onChange={this.handleChange} defaultValue={form ? form.tipo_evento_id : ''}>
                                <option key="0" value="0">Seleccionar</option>
                                {this.state.dataTipoEvento.map(elemento => (<option key={elemento.id} value={elemento.id}>{elemento.nombre}</option>))}
                            </select>
                            <br /> 
                            ...
                            <input className="form-control" type="text" name="hora_inicial" id="hora_inicial" onChange={this.handleChange} value={form ? form.hora_inicial : '00:00:00'} />
                            <br />
                            <label>Profundidad Inicial</label>
                            <input className="form-control" type="text" name="profundidad_inicial" id="profundidad_inicial" onChange={this.handleChange} value={form ? form.profundidad_inicial : ''} />
                            <br />
                            <span style={{ float: 'right' }} onClick={() => this.setFechaFinal()} ><CalendarTodayIcon icon={tableIcons.CalendarTodayIcon} />  Click para activar fecha final </span> 
                            ...
                            <div style={{ display: (showing ? "block" : "none") }}>   
                                ...
                                <input className="form-control" type="text" name="hora_final" id="hora_final" onChange={this.handleChange} value={form ? form.hora_final : '00:00:00'} />

                                <br />
                                <label>Profundidad Final</label>
                                <input className="form-control" type="text" name="profundidad_final" id="profundidad_final" onChange={this.handleChange} value={form ? form.profundidad_final : ''} />
                            </div>
                            
                            
                            <br />
                            <label htmlFor="tag">Descripci&oacute;n</label>
                            <textarea className="form-control" name="descripcion" id="descripcion" onChange={this.handleChange} value={form ? form.descripcion : ''} />

                            <br />
                            <label htmlFor="tag">Causa</label>
                            <textarea className="form-control" name="causa" id="causa" onChange={this.handleChange} value={form ? form.causa : ''} />

                            <br />
                            <label htmlFor="tag">Soluci&oacute;n</label>
                            <textarea className="form-control" name="solucion" id="solucion" onChange={this.handleChange} value={form ? form.solucion : ''} />
                ...
                <Modal isOpen={this.state.modalEliminar}>
                    <ModalBody>
                        <DeleteOutline/> Est&aacute;s seguro que deseas eliminar el registro?
                            ...
                            <br />
                            <input className="form-control" type="hidden" name="id" id="id" readOnly onChange={this.handleChange} value={form ? form.id : this.state.data.length + 1} />
                            <b><label htmlFor="nombre">Pozo: &nbsp; </label></b>
                            {form ? form.wells_nombre : ''} 
                            <br />
                            <b><label htmlFor="nombre">Descripci&oacute;n: &nbsp; </label></b>
