...
class viewVisualHistoricoDP extends React.Component {
  ...
  peticionGetTemplateConfigId = async () => {
    axios
      .get(URL + 'template_config?id=' + this.props.match.params.id)
      .then(response => {
  ...
  peticionGetEventos = async () => {
    axios
      .get(URL + 'eventos?id=' + this.state.form1.wells_id)
      .then(response => {
  ...
  peticionDataEntradaGet = () => {
    ...
    axios
      .get(URL + 'datos_entrada?iddp2=' + this.state.form1.wells_id)
      .then(response => {
        ...
        // trae el x, y = ID, DBTM
        axios
          .get(URL + 'datos_entrada?iddp=' + this.state.form1.wells_id)
          .then(response => {
            ...
            if (this.state.dataEntrada.length > 0) {
              ...
              axios
                .get(URL + 'template_track_config?idt=' + this.state.form1.id)
                .then(response => {
                  this.setState({ dataConfigTrack: response.data });
                  console.log('OK ConfigTrack ');

                  this.creaGraficaPrincipal(
                    this.state.dataRta.sort(),
                    this.state.dataEntrada2.sort()
                  );
  ...
  crearCF_LayoutHorizontal = id => {
    ...
    axios
      .get(URL + 'template_track_config?idt=' + id)
      .then(response => {
        this.setState({ dataConfigTrack: response.data });
        if (this.state.dataConfigTrack.length > 0) {
          referencia = this.state.dataConfigTrack.filter(
          ...
          if (rows_tmp > 0) {
            layout_Horizontal =
              "[{ autosize: true, uirevision: true, margin: { l: 60, r: 10, t: 10, b: 30 }, dragmode: 'zoom',showlegend: false, plot_bgcolor: 'white', paper_bgcolor: 'lightgray', xaxis: { fixedrange: false, showspikes: true, spikemode: 'across', type: 'time', tickformat: '%d %b %Y \\n %H:%M:%S ', title: 'Tiempo', nticks: 5  },";
            ...
            for (let k = 0; k < rows_tmp; k++) {
              ...
              yaxis_tmp =
                ...
                ": { title: '" +
                referencia[k].tag +
                "', autorange: 'reversed', titlefont: { size: 10, color: 'blue', }, tickfont: { size: 8.0 } },";

              datosgrafHorizont_Aux =
                datosgrafHorizont_Aux + '{ x: [' + DATETIME + '],';

              switch (referencia[k].tag) {
                case 'DBTM':
                  datosgrafHorizont_Aux =
                    datosgrafHorizont_Aux + ' y: [' + DBTM + '], ';
                  break;
                case 'DMEA':
                  datosgrafHorizont_Aux =
                    datosgrafHorizont_Aux + ' y: [' + DMEA + '], ';
                  break;
                case 'MFIA':
                  datosgrafHorizont_Aux =
                    datosgrafHorizont_Aux + ' y: [' + MFIA + '], ';
                  break;
                case 'ROPA':
                  datosgrafHorizont_Aux =
                    datosgrafHorizont_Aux + ' y: [' + ROPA + '], ';
                  break;
                case 'RPM':
                  datosgrafHorizont_Aux =
                    datosgrafHorizont_Aux + ' y: [' + RPM + '], ';
                  break;
                case 'TQA':
                  datosgrafHorizont_Aux =
                    datosgrafHorizont_Aux + ' y: [' + TQA + '], ';
                  break;
                case 'WOB':
                  datosgrafHorizont_Aux =
                    datosgrafHorizont_Aux + ' y: [' + WOB + '], ';
                  break;
                default:
                  break;
              }
            ...
            layout_Horizontal =
              layout_Horizontal +
              " grid: { rows: 10, columns: 1, subplots: [['xy'],['xy2'], ['xy3'], ['xy4'], ['xy5'],['xy6'], ['xy7'],['xy8'], ['xy9']], roworder: 'top to bottom' }," +
              yaxis_tmp +
              '}]';
          }
          ...
          datosgrafHorizont_Aux =
            '[' +
            datosgrafHorizont_Aux.substring(
              datosgrafHorizont_Aux.length - 1,
              0
            ) +
            ']';
  ...
  creaGraficaPrincipal = (rsDouglas, rsTabla) => {
    ...
    if (rsDouglas.length > 0) {
      ...
      for (let k = 0; k < rsDouglas.length - 1; k++) {
        indice = rsTabla.filter(
          item => item.datos_entrada_id === rsDouglas[k].x
        );
        ...
        if (indice.length > 0) {
          ...
          if (indice[0].operacion === 0) {
            operacion_0 = operacion_0 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_0 = operacion_0 + 'null,';
          }
          if (indice[0].operacion === 2) {
            operacion_2 = operacion_2 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_2 = operacion_2 + 'null,';
          }
          if (indice[0].operacion === 3) {
            operacion_3 = operacion_3 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_3 = operacion_3 + 'null,';
          }
          if (indice[0].operacion === 4) {
            operacion_4 = operacion_4 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_4 = operacion_4 + 'null,';
          }
          if (indice[0].operacion === 7) {
            operacion_7 = operacion_7 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_7 = operacion_7 + 'null,';
          }
          if (indice[0].operacion === 8) {
            operacion_8 = operacion_8 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_8 = operacion_8 + 'null,';
          }
          if (indice[0].operacion === 9) {
            operacion_9 = operacion_9 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_9 = operacion_9 + 'null,';
          }
          if (indice[0].operacion === 35) {
            operacion_35 = operacion_35 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_35 = operacion_35 + 'null,';
          }
          if (indice[0].operacion === 36) {
            operacion_36 = operacion_36 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_36 = operacion_36 + 'null,';
          }
          if (indice[0].operacion === 37) {
            operacion_37 = operacion_37 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_37 = operacion_37 + 'null,';
          }
          if (indice[0].operacion === 38) {
            operacion_38 = operacion_38 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_38 = operacion_38 + 'null,';
          }
          if (indice[0].operacion === 39) {
            operacion_39 = operacion_39 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_39 = operacion_39 + 'null,';
          }
          if (indice[0].operacion === 40) {
            operacion_40 = operacion_40 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_40 = operacion_40 + 'null,';
          }
          if (indice[0].operacion === 41) {
            operacion_41 = operacion_41 + "'" + indice[0].DBTMCorregida + "',";
          } else {
            operacion_41 = operacion_41 + 'null,';
          }
        }
      }
      ...
      datosGraficaPrincipal_Aux =
        ...
        '], y: [' +
        DBTM +
        '],  mode: "lines+markers",  type: "scatter", name: "DBTM" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_0 +
        '],  mode: "markers",  type: "scatter", name: "0-Undef status" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_2 +
        '],  mode: "markers",  type: "scatter", name: "2-Drilling" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_3 +
        '],  mode: "markers",  type: "scatter", name: "3-Connection" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_4 +
        '],  mode: "markers",  type: "scatter", name: "4-Reaming" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_7 +
        '],  mode: "markers",  type: "scatter", name: "7-Circulating" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_8 +
        '],  mode: "markers",  type: "scatter", name: "8-RIH" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_9 +
        '],  mode: "markers",  type: "scatter", name: "9-POOH" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_35 +
        '],  mode: "markers",  type: "scatter", name: "35-OnSurface" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_36 +
        '],  mode: "markers",  type: "scatter", name: "36-Sliding" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_37 +
        '],  mode: "markers",  type: "scatter", name: "37-POOHW/Pump" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_38 +
        '],  mode: "markers",  type: "scatter", name: "38-RIHW/Pump" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_39 +
        '],  mode: "markers",  type: "scatter", name: "39-BackReaming" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_40 +
        '],  mode: "markers",  type: "scatter", name: "40-Working Pipe" },' +
        '{ x: [' +
        DATETIME +
        '], y: [' +
        operacion_41 +
        '],  mode: "markers",  type: "scatter", name: "41-Unregister" }';
      ...
      if (rsEventos.length > 0) {
        for (let k = 0; k < rsEventos.length; k++) {
          ...
          if (fec1 === fec2) {
            ...
            datosdescrp_Eve_Aux =
              datosdescrp_Eve_Aux + "'" + rsEventos[k].descripcion + "'" + ',';
          } else {
            ...
            datosfecIni_EveShape_Aux =
              datosfecIni_EveShape_Aux + ',' + "'" + numberFecI + "'";
            datosproIni_EveShape_Aux =
              datosproIni_EveShape_Aux + "'" + pro1 + "'" + ',';
            datosdescrp_EveShape_Aux =
              datosdescrp_EveShape_Aux +
              "'" +
              rsEventos[k].descripcion +
              "'" +
              ',';
          }
        }
        ...
        if (datosfecIni_EveShape_Aux.length > 0) {
          datosfecIni_EveShape_Aux = datosfecIni_EveShape_Aux.substring(
            1,
            datosfecIni_EveShape_Aux.length
          );
          datosproIni_EveShape_Aux = datosproIni_EveShape_Aux.substring(
            datosproIni_EveShape_Aux.length - 1,
            1
          );
          datosdescrp_EveShape_Aux = datosdescrp_EveShape_Aux.substring(
            datosdescrp_EveShape_Aux.length - 1,
            1
          );
        }
        ...
        if (eventShapes !== '') {
          this.setState({
            shapes: eventShapes.substring(eventShapes.length - 1, 0),
          });
