...
class viewVisualConfig extends Component {
    ...
    peticionPost = async () => {
        ...
        await axios.post(urlAuxiliar3, this.state.form1).then(response => {
    ...
    peticionArchivoLasGet2 = (ida, idr) => {
        axios.get(urlAuxiliar1 + '?idar=' + ida + '&idr=' + idr).then(response => {
            ...
            else { this.setState({ visual_variable: false });
            }
            ReactDOM.render();
        }).catch(error => {
            console.log(error.message);
        ...
        return (
            <div className="App">
                ...
                <div className="form-group col-8" style={{ float: 'left', padding: '0 0 0 70px' }}> 
                    <br />
                    <h5> Nueva parametrizaci&oacute;n de la visualizaci&oacute;n de la gr&aacute;fica principal</h5>                      
                        <hr></hr>
                        <h6>   <AddBox icon={tableIcons.Add} />  1: Selecci&oacute;n del Pozo y fuente de origen de datos </h6>
                        <hr></hr>        
                    <div style={{ float: 'left', padding: '15px 0 0 60px' }} className="form-group">
                        <input className="form-control" type="hidden" name="contador" id="contador" value={form ? form.contador : ''} />                        
                        Seleccion el Wells
                        <select name="wells_id" id="wells_id" className="form-control" value={ form.wells_id } onChange={this.handleChange}>
                        ...
                        </select> 
                        {
                            this.state.visual_ajax1 ? (
                                <div id="ajax_visual1">
                                    <span className="badge badge-success">Datos de telemetria: Encontrados con los Mnemonicos: {this.state.dataTelemetria.map(elemento => (elemento.nameMnemonic))}  </span>
                                    <br />
                                </div>) : (<div><span className="badge badge-light">Datos de telemetria: NO Encontrados </span><br /></div>)  
                        }
                        <br />
                        { this.state.visual_ajax2 ? (
                            ...
                            </div>) : (<div><span className="badge badge-light">Archivos .LAS: NO Encontrados</span><br /></div>)
                        }
                        <div id="nemonicarc" >
                        ...
                        <div style={{ float: 'left', padding: '15px 0 0 60px'  }} className="form-group col-12">
                            <div className="container">
                                ...
                                <div className="row">
                                    ...
                                    {this.state.visual_ajax1 && this.state.form.datetime_estt ? (<div className="col-4"><input type="radio" value="1" name="datetime" onChange={this.handleChange21} /> Telemetria</div>) : (<div className="col-2"></div>)}
                                    {this.state.visual_ajax2 && this.state.select_archivo && this.state.form.datetime_est ? (<div className="col-4"><input type="radio" value="2" name="datetime" onChange={this.handleChange21} /> Archivo .Las</div>) : (<div className="col-2"></div>)}
                                </div> 

                                <div className="row">
                                    <div className="col-2">DMEA</div>
                                    {this.state.visual_ajax1 && this.state.form.dmea_estt ? (<div className="col-4"><input type="radio" value="1" name="dmea" onChange={this.handleChange21} /> Telemetria</div>) : (<div className="col-2"></div>)}
                                    {this.state.visual_ajax2 && this.state.select_archivo && this.state.form.dmea_est ? (<div className="col-4"><input type="radio" value="2" name="dmea" onChange={this.handleChange21} /> Archivo .Las</div>) : (<div className="col-2"></div>)}
                                </div>

                                <div className="row">
                                    <div className="col-2">DBTM</div>
                                    {this.state.visual_ajax1 && this.state.form.dbtm_estt ? (<div className="col-4"><input type="radio" value="1" name="dbtm" onChange={this.handleChange21} /> Telemetria</div>) : (<div className="col-2"></div>)}
                                    {this.state.visual_ajax2 && this.state.select_archivo && this.state.form.dbtm_est ? (<div className="col-4"><input type="radio" value="2" name="dbtm" onChange={this.handleChange21} /> Archivo .Las</div>) : (<div className="col-2"></div>)}
                                </div>

                                <div className="row">
                                    <div className="col-2">RPM</div>
                                    {this.state.visual_ajax1 && this.state.form.rpm_estt ? (<div className="col-4"><input type="radio" value="1" name="rpm" onChange={this.handleChange21} /> Telemetria</div>) : (<div className="col-2"></div>)}
                                    {this.state.visual_ajax2 && this.state.select_archivo && this.state.form.rpm_est ? (<div className="col-4"><input type="radio" value="2" name="rpm" onChange={this.handleChange21} /> Archivo .Las</div>) : (<div className="col-2"></div>)}
                                </div>

                                <div className="row">
                                    <div className="col-2">ROPA</div>
                                    {this.state.visual_ajax1 && this.state.form.ropa_estt ? (<div className="col-4"><input type="radio" value="1" name="ropa" onChange={this.handleChange21} /> Telemetria</div>) : (<div className="col-2"></div>)}
                                    {this.state.visual_ajax2 && this.state.select_archivo && this.state.form.ropa_est ? (<div className="col-4"><input type="radio" value="2" name="ropa" onChange={this.handleChange21} /> Archivo .Las</div>) : (<div className="col-2"></div>)}
                                </div>

                                <div className="row">
                                    <div className="col-2">MFIA</div>
                                    {this.state.visual_ajax1 && this.state.form.mfia_estt ? (<div className="col-4"><input type="radio" value="1" name="mfia" onChange={this.handleChange21} /> Telemetria</div>) : (<div className="col-2"></div>)}
                                    {this.state.visual_ajax2 && this.state.select_archivo && this.state.form.mfia_estt ? (<div className="col-4"><input type="radio" value="2" name="mfia" onChange={this.handleChange21} /> Archivo .Las</div>) : (<div className="col-2"></div>)}
                                </div>

                                <div className="row">
                                    <div className="col-2">TQA</div>
                                    {this.state.visual_ajax1 && this.state.form.tqa_estt ? (<div className="col-4"><input type="radio" value="1" name="tqa" onChange={this.handleChange21} /> Telemetria</div>) : (<div className="col-2"></div>)}
                                    {this.state.visual_ajax2 && this.state.select_archivo && this.state.form.tqa_est ? (<div className="col-4"><input type="radio" value="2" name="tqa" onChange={this.handleChange21} /> Archivo .Las</div>) : (<div className="col-2"></div>)}
                                </div>

                                <div className="row">
                                    <div className="col-2">WOB</div>
                                    {this.state.visual_ajax1 && this.state.form.wob_estt ? (<div className="col-4"><input type="radio" value="1" name="wob" onChange={this.handleChange21} /> Telemetria</div>) : (<div className="col-2"></div>)}
                                    {this.state.visual_ajax2 && this.state.select_archivo && this.state.form.wob_est ? (<div className="col-4"><input type="radio" value="2" name="wob" onChange={this.handleChange21} /> Archivo .Las</div>) : (<div className="col-2"></div>)}
                                </div>
                        ...
                        (
                            <div style={{ float: 'left', padding: '15px 0 0 60px'  }} className="form-group col-12">
                                ...
                                <div className="container">
                                    ...
                                    <div className="col-4"><button className="btn btn-success" onClick={() => this.peticionPost()} style={{ float: 'center' }} ><Save icon={tableIcons.Save} /> Guardar Configuraci&oacute;n </button></div>) :
                                            (<div className="alert alert-danger col-6" role="alert">No estan las variables necesarias para configurar la gr&aacute;fica principal <br /></div>) 
                                }
                                <div className="col-2"><Link className="btn btn-danger"  to="/visual_config_lista"  > <CancelIcon icon={tableIcons.Add} /> Cancelar</Link></div>
                                    </div>
                                </div>
                                        <hr></hr>
                        </div>)
