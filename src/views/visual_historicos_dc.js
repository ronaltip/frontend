...
const PORCENTAJE_INCREMENTO_TRACK_VERTICAL = 0.7;
...
class viewVisualHistoricoDC extends React.Component {
    ...
    if (id === 0) {
      ...
      axios
        ...
        .then(response => {
          ...
          const requestCurvas = axios.get(
            URL + 'tipo_curvas/templates/' + template.template_id
          );
          //Datos Del Pozo
          const requestDatos = axios.get(
            URL + 'datos_wits/wells/' + template.wells_id
          );
          //Conveción de colores de las operaciones
          const requestConvencion = axios.get(
            URL + 'convencion_datos_operacion'
          );
          //Archivos asociados
          //const requestArchivos = axios.get(URL + "archivo_encabezado/" + template.wells_id);

          axios
            .all([requestCurvas, requestDatos, requestConvencion])
            .then(
              axios.spread((...response) => {
                const curvas = response[0];
                const dataWits = response[1].data.filter(
                  d =>
                    Number(d.DBTM) < 15000 &&
                    Number(d.DMEA) < 15000 &&
                    Number(d.DBTM) > 0 &&
                ...
                const datosToSimplyfy = dataWits
                  .map(obj => ({ x: Number(obj.id), y: Number(obj.DBTM) }))
                  .sort(x => x.id);
                let dataRta = Simplify(datosToSimplyfy, 0.9075);

                this.creaGraficaPrincipal_Full_v2(
                  convencion,
                  dataRta,
                  dataWits
                );
  ...
  seleccionarRegistro = templateconfig => {
    ...
    if (templateconfig[0].datetime === 'T') {
      datetimeest = 1;
    } else {
      datetimeest = 2;
      tmpVal = 0;
    }
    if (templateconfig[0].dmea === 'T') {
      dmeaest = 1;
    } else {
      dmeaest = 2;
      tmpVal = 0;
    }
    if (templateconfig[0].dbtm === 'T') {
      dbtmest = 1;
    } else {
      dbtmest = 2;
      tmpVal = 0;
    }
    if (templateconfig[0].rpm === 'T') {
      rpmest = 1;
    } else {
      rpmest = 2;
      tmpVal = 0;
    }
    if (templateconfig[0].ropa === 'T') {
      ropaest = 1;
    } else {
      ropaest = 2;
      tmpVal = 0;
    }
    if (templateconfig[0].mfia === 'T') {
      mfiaest = 1;
    } else {
      mfiaest = 2;
      tmpVal = 0;
    }
    if (templateconfig[0].tqa === 'T') {
      tqaest = 1;
    } else {
      tqaest = 2;
      tmpVal = 0;
    }
    if (templateconfig[0].wob === 'T') {
      wobest = 1;
    } else {
      wobest = 2;
      tmpVal = 0;
  ...
  peticionDataEntradaGet_OLD = () => {
    ...
    axios
      .get('?iddp2=' + this.state.form1.wells_id)
      .then(response => {
        ...
        // trae el x, y = ID, DBTM
        axios
          .get('?iddp=' + this.state.form1.wells_id)
          .then(response => {
            ...
            if (this.state.dataEntrada.length > 0) {
              ...
              axios
                .get('urlAuxiliar1' + '?idt=' + this.state.form1.id)
                .then(response => {
                  this.setState({ dataConfigTrack: response.data });
                  console.log('OK ConfigTracks ');

                  this.creaGraficaPrincipal_Full(
                    this.state.dataRta,
                    this.state.dataEntrada2
                  );
                  this.crearCF_LayoutHorizontal();
                  this.crearCF_LayoutVertical();
  ...
  crearCF_LayoutHorizontal = () => {
    ...
    if (this.state.dataConfigTrack.length > 0) {
      let referencia = this.state.dataConfigTrack.filter(
        item => item.referencia_id === 1
      );
      ...
      console.log('No hay tracks horizontales');
    }
  ...
  crearCF_LayoutVertical = () => {
    ...
    if (this.state.dataConfigTrack.length > 0) {
      ...
      if (referencia.length > 0) {
        axios
          .get(
            'urlAuxiliar6' + '?eid=' + this.state.form1.archivo_encabezado_id
          )
          .then(response => {
            let i = 1;
      ...
      console.log('No hay tracks verticales');
    }
  ...
  creaGraficaPrincipal_Full = (rsDouglas, rsTabla) => {
    ...
    datosGraficaPrincipal.push({
      ...
      type: 'scatter',
      name: '0-Undef status',
      marker: { color: hexRgb('#FFFF00', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_2,
      ...
      type: 'scatter',
      name: '2-Drilling',
      marker: { color: hexRgb('#8497B0', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_3,
      ...
      type: 'scatter',
      name: '3-Connection',
      marker: { color: hexRgb('#00B050', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_4,
      ...
      type: 'scatter',
      name: '4-Reaming',
      marker: { color: hexRgb('#BF9000', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_7,
      ...
      type: 'scatter',
      name: '7-Circulating',
      marker: { color: hexRgb('#FF0000', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_8,
      ...
      type: 'scatter',
      name: '8-RIH',
      marker: { color: hexRgb('#00FFFF', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_9,
      ...
      type: 'scatter',
      name: '9-POOH',
      marker: { color: hexRgb('#548235', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_35,
      ...
      type: 'scatter',
      name: '35-OnSurface',
      marker: { color: hexRgb('#767171', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_36,
      ...
      type: 'scatter',
      name: '36-Sliding',
      marker: { color: hexRgb('#FF00FF', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_37,
      ...
      type: 'scatter',
      name: '37-POOHW/Pump',
      marker: { color: hexRgb('#FF9933', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_38,
      ...
      type: 'scatter',
      name: '38-RIHW/Pump',
      marker: { color: hexRgb('#D6DCE5', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_39,
      ...
      type: 'scatter',
      name: '39-BackReaming',
      marker: { color: hexRgb('#DBDBDB', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_40,
      mode: 'markers',
      type: 'scatter',
      name: '40-Working Pipe',
      marker: { color: hexRgb('#FBE5D6', { format: 'css' }) },
    });
    datosGraficaPrincipal.push({
      x: DATETIME,
      y: operacion_41,
      ...
      type: 'scatter',
      name: '41-Unregister',
      marker: { color: hexRgb('#FFE699', { format: 'css' }) },
    });
  ...
  PlotTrackVerticalOnHover = e => {
    ...
    Plotly.Fx.hover('plotDept', [{ curveNumber: 0, yval: e.yvals[0] }], ['xy']);
  };
    ...
    return (
      <div className="">
        ...
        <div className="container-fluid">
          <div className="row" style={{ backgroundColor: 'rgb(200,200,200)' }}>
            <div className="col-md-12 col-lg-12 text-center">
              <label>
                <b>Datos históricos Pozo:</b> {this.state.form1.nombre_pozo}
              </label>
                          ...
                          {this.state.isLoadedHorizontal ? (
                            <>
                              <Plot
                                ...
                                onHover={e =>
                                  this.PlotTrackHorizontalOnHover(e)
                                }
                                onUnhover={e =>
                                  this.PlotTrackHorizontalOnUnHover(e)
                                }
                              />
                              <div className="row">
                                <div className="col-md-12">
                                  <button
                                    ...
                                    onClick={() =>
                                      this.setState({ openModalTH: true })
                                    }
                                    style={{
                                      position: 'fixed',
                                      bottom: '4px',
                                    ...
                                    title="Activar/Desactivar Tracks Horizontales"
                                  >
                          ...
                          <div className="row">
                            <div className="col-md-12 mt-2">
                              <button
                                ...
                                onClick={() =>
                                  this.setState({ openModalTV: true })
                                }
                                title="Activar/Desactivar Tracks Verticales"
                              >
          ...
          <ModalBody>
            ...
            <div className="row" onClick={() => this.setFechaFinal()}>
              <div className="col-md-12 text-center">
                <div className="form-group">
                  <CalendarTodayIcon style={{ cursor: 'pointer' }} /> Click para
        ...
        <Modal isOpen={this.state.openModalTH}>
          <ModalHeader>
            <div>
              <label>Activar / Desactivar Track Horizontal</label>
            </div>
          </ModalHeader>
          <ModalBody>
            {this.state.isLoadedHorizontal
              ? this.state.tracksHor.map((t, index) => (
                  <div key={`div_th_${index}`} className="row">
                    <div className="col-md-12">
                      <input
                        key={`chk_th_${index}`}
                        type="checkbox"
          ...
          <ModalFooter>
            <button
              className="btn btn-secondary"
              onClick={() => this.setState({ openModalTH: false })}
            >
              Cerrar
            </button>
          </ModalFooter>
        ...
        <Modal isOpen={this.state.openModalTV}>
          <ModalHeader>
            <div>
              <label>Activar / Desactivar Track Vertical</label>
            </div>
          </ModalHeader>
          <ModalBody>
            {this.state.isLoadedVertical
              ? this.state.tracksVer.map((t, index) => (
                  <div key={`div_tv_${index}`} className="row">
                    <div className="col-md-12">
                      <input
                        key={`chk_tv_${index}`}
                        type="checkbox"
          ...
          <ModalFooter>
            <button
              className="btn btn-secondary"
              onClick={() => this.setState({ openModalTV: false })}
            >
              Cerrar
            </button>
          </ModalFooter>
