...
class viewUnidades extends Component {
    ...
    peticionPost = async () => {
        ...
        await axios.post(URL + 'unidades', this.state.form).then(response => {
            this.modalInsertar();
            this.peticionGet();
        }).catch(error => {
            console.log(error.message);
        ...
        return (
            <div className="App">
                ...
                <div className="containerCuatro">
                    <button className="btn btn-success" onClick={() => { this.setState({ form: null, tipoModal: 'insertar' }); this.modalInsertar() }}> <AddBox icon={tableIcons.Add} /> Agregar Unidad</button>
                </div>
